...
class Goods extends BaseController
{
    ...
    {
        $title = "获取品牌专区信息";
        $ap_id = request()->post("ap_id", '');//广告图片id
        ...
        // 品牌专区广告位
        $brand_adv = $platform->getPlatformAdvPositionDetail(1162);//获取品牌专区传id
        ...
        $goods_category_list_1 = $goods_category->getGoodsCategoryList(1, 0, [  //分页参数
            "is_visible" => 1,
            "level" => 1
        ]);
    ...
    public function getBrandAdvPosition()
    {
        $title = "品牌专区广告位";
        $platform = new Platform();
        $pid      = request()->post('ap_id',"");
        // 品牌专区广告位
        if (empty($pid)) {
            return $this->outMessage($title, null, '-50', "无法获取品牌专区广告位!");
        }
        $brand_adv = $platform->getPlatformAdvPositionDetail($pid);
       return $this->outMessage($title, $brand_adv);
    }
